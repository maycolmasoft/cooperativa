...
<html lang="es">
	...
	function load_usuarios(pagina){
		...
		$.ajax({
			url:"<?php echo $helper->url("ReporteUsuarios","index");?>",
            type : "POST",
            async: true,			
			data: con_datos,
			 beforeSend: function(objeto){
			$("#usuarios").html('<img src="view/images/ajax-loader.gif"> Cargando...');
			},
			success:function(data){
				$(".div_usuarios").html(data).fadeIn('slow');
				$("#usuarios").html("");
			}
		})
	}
     ...
     <script>
	       	$(document).ready(function(){ 	
				$( "#correo_usuarios" ).autocomplete({
      				source: "<?php echo $helper->url("ReporteUsuarios","AutocompleteCorreo"); ?>",
      				minLength: 1
    			});
	...
	<script>
	       	$(document).ready(function(){ 	
				$( "#nombre_usuarios" ).autocomplete({
      				source: "<?php echo $helper->url("ReporteUsuarios","AutocompleteNombre"); ?>",
      				minLength: 1
    			});
  ...
  <div class="row" style="background-color: #ffffff;">
      ...
      <form action="<?php echo $helper->url("ReporteUsuarios","index"); ?>" method="post" enctype="multipart/form-data"  class="col-lg-12" target="_blank">
	  ...
	  <!-- 
       ...
       <?php } else {?>
		 ...
		 <h4 style="color:#ec971f;"></h4>
			  <div>					
					<div id="usuarios" style="position: absolute;	text-align: center;	top: 10px;	width: 100%;display:none;"></div><!-- Carga gif animado -->
